...
def playgame():
    ...
    while running:
        ...
        for enemy in enemies:
            enemy.update(dt)
            enemy.draw()
            if enemy.checkCollision(groundObj.playerFloor):
                enemies.remove(enemy)
                gui.lives -= 1
                if gui.lives == 0:
                    GameOver.game_over(gui)
                    running = False
        ...
        elif gui.difficultyLevel == 1 and len(enemies) < 10 and pygame.time.get_ticks() % 2000 < dt * 1000:
            for _ in range(2):
                new_enemy = EnemyStuff.Enemy(screen)
                new_enemy.speed = 50
        elif gui.difficultyLevel == 2 and len(enemies) < 15 and pygame.time.get_ticks() % 1500 < dt * 1000:
            for _ in range(2):
                new_enemy = EnemyStuff.Enemy(screen)
                new_enemy.speed = 75
                enemies.append(new_enemy)
